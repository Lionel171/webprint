...
import Search from '../../components/chat/group/Search'
    ...
    <>
      ...
      <div className="bg-[#282C35!] scrollbar-hide z-10 h-[100vh]  lg:w-[90%] lg:mx-auto overflow-y-hidden shadow-2xl">
        ...
        <div className='flex'>
          {
            !showProfile ?
              <div className="md:flex md:flex-col sm:min-w-[30%] h-[100vh] md:h-[98.6vh] bg-[#ffff] relative">

                <div className='h-[61px] px-4'>
                  <div className='flex'>
                    <a className='flex items-center relative  -top-4 block h-[90px]' href='/' >
                      <h3 className='text-[20px] text-[#1f2228] font-body font-extrabold tracking-wider sm:block hidden'>Messages</h3>
                    </a>
                  </div>
                  <div className='absolute top-4 right-5 flex items-center gap-x-3'>
                    ...
                    <button onClick={() => dispatch(setShowProfile(true))} className='flex items-center gap-x-1 relative'>
                      <img className='w-[28px] h-[28px] rounded-[25px]' src={user.profile_image ? API_URL + '/' + user.profile_image : DefaultImage} alt="" />
