...
import Loading from '../../components/chat/ui/Loading';
import data from '@emoji-mart/data'
import Picker from '@emoji-mart/react'
import { getChatName } from '../../utils/logics';
import Typing from '../../components/chat/ui/Typing';
    ...
    <>
      {
        activeChat ?
          <div className={props.className}>
            <div className='flex justify-between items-center px-5 bg-[#ffff] w-[100%]'>
              <div className='flex items-center gap-x-[10px]'>
                <div className='flex flex-col items-start justify-center'>
                  <h5 className='text-[17px] text-[#2b2e33] font-bold tracking-wide'>{getChatName(activeChat, user)}</h5>
            ...
            <div className='absolute left-[5%] w-[90%] bottom-[8%]'>
              {
                showPicker && <Picker data={data} onEmojiSelect={(e) => setMessage(message + e.native)} />
              }
              <div className='border-[1px] border-[#aabac8] px-6 py-3  h-[50px] rounded-t-[10px]  ml-3 mr-3'>
              ...
              <div className='border-x-[1px] border-b-[1px] bg-[#f8f9fa] border-[#aabac8] px-6 py-3 rounded-b-[10px] h-[50px]  ml-3 mr-3'>
                {/* {
          ...
          <div className={props.className}>
            <div className='relative'>
              <div className=' flex flex-col items-center justify-center gap-y-3 mt-5'>
                <img className='w-[50px] h-[50px] rounded-[25px]' alt="User profile" src={user.profile_image ? API_URL + '/' + user.profile_image : DefaultImage} />
                <h3 className='text-[#111b21]  font-medium tracking-wider'>Welcome <span className='text-[#166e48]  font-bold'> {user.contact_person}</span></h3>
              </div>
            </div>
          </div>
